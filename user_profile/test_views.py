...
from user_profile.models import UserProfile, SellerProfile, SpecialistProfile, VerificationDocument
...
class ProfilePermissionsTests(TestCase):
    def setUp(self):
        ...
        # Создаем другого пользователя для тестов доступа
    ...
    def test_profile_settings_requires_auth(self):
        """Тест: страница настроек профиля требует авторизации"""
        # Пробуем получить доступ без авторизации
        response = self.client.get(reverse('user_profile:settings'))
        self.assertEqual(response.status_code, 302)  # Редирект на страницу входа
        self.assertTrue(response.url.startswith('/auth/'))  # Обновленный URL

    def test_own_profile_access(self):
        """Тест: пользователь имеет доступ к своему профилю"""
        self.client.login(phone='+79991234567', password='testpass123')
        response = self.client.get(reverse('user_profile:settings'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'user_profile/settings.html')

    def test_other_profile_access_forbidden(self):
        """Тест: пользователь не имеет доступа к чужому профилю"""
        self.client.login(phone='+79991234567', password='testpass123')
        # Пытаемся получить доступ к профилю другого пользователя
        response = self.client.get(
            reverse('user_profile:update_seller_profile', kwargs={'pk': self.other_user.id})
        )
        self.assertEqual(response.status_code, 403)  # Forbidden

    def test_seller_profile_creation_requires_verification(self):
        """Тест: создание профиля продавца требует верификации"""
        self.client.login(phone='+79991234567', password='testpass123')
        
        # Пробуем создать профиль продавца без верификации
        response = self.client.post(reverse('user_profile:create_seller_profile'), {
            'seller_type': 'individual',
            'description': 'Test seller'
        })
        ...
        # Пробуем создать профиль продавца после верификации
        response = self.client.post(reverse('user_profile:create_seller_profile'), {
            'seller_type': 'individual',
            'description': 'Test seller'
        })
        self.assertEqual(response.status_code, 302)  # Успешное создание и редирект
        self.assertTrue(SellerProfile.objects.filter(user=self.user).exists())

    def test_profile_deletion(self):
        """Тест: пользователь может удалить только свой профиль"""
        self.client.login(phone='+79991234567', password='testpass123')
        
        # Пытаемся удалить чужой профиль
        response = self.client.post(
            reverse('user_profile:delete_profile', kwargs={'pk': self.other_user.id})
        )
        self.assertEqual(response.status_code, 403)  # Forbidden
        
        # Удаляем свой профиль
        response = self.client.post(
